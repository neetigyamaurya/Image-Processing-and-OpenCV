...
# Histogram equalization is good when of the images is confined to a particular region. It accepts gray scale image.
equ = cv2.equalizeHist(image)
res = np.hstack((image, equ))  # Optional step done so that the original image and the new output are joined
...
res = np.hstack((image, equ, cl1))  # Optional step done so that the original image and the new output are joined
plt.plot(hist)
plt.title("CLAHE")
plt.show()
